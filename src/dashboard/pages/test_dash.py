...
ecom_country = (
    ecom_sales.groupby("Country")["OrderValue"]
    .agg(["sum", "count"])
    .reset_index()
            ...
            children=[
                html.Div(
                    children=[
                        ...
                        dcc.Dropdown(
                            id="minor_cat_dd", style={"width": "200px", "margin": "0 auto"}
                        ),
                    ],
                    style={
                        "width": "350px",
                ...
                html.Div(
                    children=[dcc.Graph(id="sales_line")],
...
@app.callback(Output("sales_line", "figure"), Input("minor_cat_dd", "value"))
def update_line(minor_cat):
    ...
    ecom_line = (
        ecom_line.groupby("Year-Month")["OrderValue"].agg("sum").reset_index(name="Total Sales ($)")
    )
    line_graph = px.line(
        ecom_line,
        x="Year-Month",
